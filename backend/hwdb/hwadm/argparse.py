...
def _add_new_system_parser(subparsers: _SubParsersAction):
    ...
    parser.add_argument("group", type=group, nargs="?", help="the system group")
    parser.add_argument(
        "-n", "--amount", type=int, default=1, help="amount of systems to add"
    )
    parser.add_argument(
        "-d", "--deployment", type=deployment, help="the deployment of the system"
    )
    parser.add_argument("--key", help="the OpenVPN key to use")
    parser.add_argument("--mtu", type=int, help="the MTU for the OpenVPN tunnel")
    parser.add_argument(
        "-o",
        "--operating-system",
    ...
    parser.add_argument("--monitor", type=int, help="set explicit monitoring flag")
    parser.add_argument(
...
def _add_deployment_add_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        ...
        "--connection",
        type=connection,
        default=Connection.DSL,
        help="the internet connection on site",
    )
    parser.add_argument("-s", "--scheduled", type=date, help="the scheduled date")
    parser.add_argument("-a", "--annotation", help="a descriptive annotation")
    parser.add_argument(
        "--testing", action="store_true", help="flag the deployment as testing"
    )
...
def _add_new_deployment_parser(subparsers: _SubParsersAction):
    ...
    parser.add_argument("customer", type=customer, help="the owner of the deployment")
...
def _add_dataset_parser(subparsers: _SubParsersAction):
    ...
    parser.add_argument("dataset", type=deployment, nargs="?", help="the dataset")


def _add_hooks_parser(subparsers: _SubParsersAction):
    """Adds a parser for running hooks."""

    parser = subparsers.add_parser("run-hooks", help="run post-transaction hooks")
    parser.add_argument(
        "-H",
        "--hooks",
        nargs="*",
...
def _add_toggle_updating_parser(subparsers: _SubParsersAction):
    ...
    parser.add_argument("system", nargs="*", type=int, help="systems to toggle")

...
class Display(BaseModel):  # pylint: disable=R0903
    ...
    def select(cls, *args, cascade: bool = False, **kwargs) -> Select:
        ...
        return (
            ...
            .join(dep_customer, join_type=JOIN.LEFT_OUTER)
            .join(dep_company, join_type=JOIN.LEFT_OUTER)
            .join_from(
                Deployment,
                dep_address,
                on=Deployment.address == dep_address.id,
                join_type=JOIN.LEFT_OUTER,
            )
            .join_from(
                Deployment,
            ...
            .join_from(
                dataset,
                ds_lpt_address,
                on=dataset.lpt_address == ds_lpt_address,
                join_type=JOIN.LEFT_OUTER,
            )
            .join_from(
                # OpenVPN
